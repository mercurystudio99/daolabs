...
export const discountRateFrom = (percentage: string): BigNumber =>
	BigNumber.from(Math.floor((parseFloat(percentage) * MAX_DISCOUNT_RATE) / 100));
...
export const preciseFormatSplitPercent = (percentPerBillion: number): number =>
	(percentPerBillion / SPLITS_TOTAL_PERCENT) * 100;
...
export const reservedRateFrom = (percentage: string): BigNumber =>
	BigNumber.from(Math.floor((parseFloat(percentage) * MAX_RESERVED_RATE) / 100));
...
export const redemptionRateFrom = (percentage: string): BigNumber =>
	BigNumber.from(Math.floor((parseFloat(percentage) * MAX_REDEMPTION_RATE) / 100));
...
export const issuanceRateFrom = (issuanceRate: string): string =>
	constants.WeiPerEther.mul(issuanceRate).toString();
