...
const parameters: {
...
}[] = [
	{
		name: 'version',
		bits: 8,
		parser: (val: BigNumber) => val.toNumber(),
	},
	{
		name: 'global',
		bits: 16,
...
export const decodeV2FundingCycleMetadata = (packedMetadata: BigNumber): V2FundingCycleMetadata => {
	const metadata = parameters.reduce((data, parameter, i) => {
		...
		let shiftRightBits: number;
		if (i === 0) {
			shiftRightBits = 0;
		} else {
			shiftRightBits = parameters.slice(0, i).reduce((acc, p) => {
				acc += p.bits;
				return acc;
			}, 0);
		}
...
export const getUnsafeV2FundingCycleProperties = (
	...
	const metadata = fundingCycleMetadata || decodeV2FundingCycleMetadata(fundingCycle.metadata);
...
export type FundingCycleDetail = {
	id: string;
	label: string;
	value: string;
	info?: string;
