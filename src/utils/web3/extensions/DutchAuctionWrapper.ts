 ...
 * @param startingPrice Auction starting price, the "high" price.
 * @param endingPrice Auction ending price, the "low" price.
...
export async function create<T extends boolean = false>(
	...
	const DutchAuctionHouse = await contracts[platform][get(readNetwork).alias].DutchAuctionHouse();
	const contract = new ethers.Contract(
		contractAddress,
		DutchAuctionHouse.abi,
...
/**
 * Settles an auction on the condition that either the required price has been met or the auction duration has elapsed. This action only transfers the token on sale to the bidder or back to the seller. To additionally transfer auction proceeds, call `distributeProceeds`.
 *
 * @param contractAddress Address of the contract to invoke.
 * @param collectionAddress ERC721 contract address.
 * @param item Token id.
...
export async function timeLeft(
	...
	const { abi } = await contracts[platform][get(readNetwork).alias].DutchAuctionHouse();
