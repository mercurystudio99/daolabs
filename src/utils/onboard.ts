...
export function coinbaseWallet({ darkMode = false } = {}): WalletInit {
	return () => ({
		label: 'Coinbase',
		getIcon: () =>
			new Promise<string>((r) =>
				r(`<svg width="100%" height="100%" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
		...
		getInterface: async ({
			...
			chains: { rpcUrl: string; id: string }[];
			...
			const on: typeof coinbaseWalletProvider.on =
				coinbaseWalletProvider.on.bind(coinbaseWalletProvider);
...
export function gnosis({ whitelistedDomains = [/gnosis-safe.io/] }): WalletInit {
	return () => ({
		label: 'Gnosis Safe',
		getIcon: () =>
			new Promise<string>((r) =>
				r(`<svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 440 440">
		...
		getInterface: async () => {
			...
			const SafeAppProviderConstructor = SafeAppsSDK;
			const appsSdk = new SafeAppProviderConstructor({
				allowedDomains: whitelistedDomains,
			});
			const safe: SafeInfo = await Promise.race([
				appsSdk.safe.getInfo(),
				new Promise<SafeInfo>((resolve) => setTimeout(resolve, 200)),
			]);
