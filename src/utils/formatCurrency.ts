...
export class CurrencyUtils {
	...
	constructor(usdPerEth: number | undefined) {
		if (!usdPerEth) {
			console.info('Failed to construct CurrencyUtils, received a usdPerEth value of', usdPerEth);
			return;
		}
	...
	usdToWei = (amount: number | string | undefined, precision = 8) => {
		...
		try {
			return parseWad(
				((typeof amount === 'string' ? parseFloat(amount) : amount) / this.usdPerEth).toFixed(
					precision,
				),
			);
		} catch (e) {
			console.error("Couldn't convert USD amount", amount.toString(), 'to wei', e);
		}
	};
	...
	wadToCurrency = (
		...
		if (targetCurrency === 'USD') return parseWad(this.weiToUsd(amount)?.toString());
