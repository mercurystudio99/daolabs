...
import {
	parseNftContractJson,
	parseNftHolderJson,
	parseNftTokenJson,
	type NftContract,
	type NftHolder,
	type NftToken,
} from '$models/subgraph-entities/v2/nft-contract';
import type {
...
export const formatGraphQuery = <E extends EntityKey, K extends EntityKeys<E>>(
	...
	const formatWhere = (where: WhereConfig<E>) =>
		`${String(where.key)}${typeof where.operator === 'string' ? `_${where.operator}` : ''}:${
...
export function formatGraphResponse<E extends EntityKey>(
	...
	switch (entity) {
		...
		case 'mintTokensEvent':
			if ('mintTokensEvent' in response) {
				// @ts-ignore
				return response.mintTokensEvent.map(parseMintTokensEventJson);
			}
			break;
		...
		case 'distributePayoutsEvent':
			if ('distributePayoutsEvent' in response) {
				// @ts-ignore
				return response.distributePayoutsEvent.map(parseDistributePayoutsEventJson);
			}
			break;
		...
		case 'deployETHERC20ProjectPayerEvent':
			...
			break;
		case 'nftContract':
			if ('nftContracts' in response) {
				// @ts-ignore
				return response.nftContracts.map(parseNftContractJson);
			}
			break;
		case 'nftToken':
			if ('nftTokens' in response) {
				// @ts-ignore
				return response.nftTokens.map(parseNftTokenJson);
			}
			break;
		case 'nftHolder':
			if ('nftHolders' in response) {
				// @ts-ignore
				return response.nftHolders.map(parseNftHolderJson);
			}
			break;
	}
...
export async function querySubgraph<E extends EntityKey, K extends EntityKeys<E>>(
	...
	const subgraphUrl = `${import.meta.env.VITE_FIREBASE_FUNCTIONS_URL as string}/app/subgraph/${
...
export async function querySubgraphRaw<E extends EntityKey>(query: string) {
	const subgraphUrl = `${import.meta.env.VITE_FIREBASE_FUNCTIONS_URL as string}/app/subgraph/${
		readNetwork.get().alias
	}`;
	if (!subgraphUrl) {
	...
	if (!query) return [] as unknown as SubgraphQueryReturnTypes[E];
