...
export type ProtocolLogJson = Omit<ProtocolLog, 'volumePaid' | 'volumeRedeemed' | 'v1' | 'v2'> & {
	volumePaid: string;
...
export const parseProtocolLogJson = (json: Partial<ProtocolLogJson>): Partial<ProtocolLog> => ({
	...json,
	volumePaid: json.volumePaid ? BigNumber.from(json.volumePaid) : undefined,
	volumeRedeemed: json.volumeRedeemed ? BigNumber.from(json.volumeRedeemed) : undefined,
