<script lang="ts">
	...
	import PfpLayering from '$lib/components/minter/preview/PFPLayering.svelte';
	...
	let dragX = 0;
	let dragY = 0;
	let dragInitX: number;
	let dragInitY: number;
	...
	const handleDragStart = (e: DragEvent) => {
		...
		dragInitX = e.clientX - dragX;
		dragInitY = e.clientY - dragY;
	};
...
<div class="wrapper" on:wheel={handleWheel}>
	<ul class="controls">
		...
		<li on:click={() => (zoom > 1 ? (zoom += 1) : (zoom *= 2))} on:keydown>
			<Icon name="zoomIn" />
		</li>
		<li on:click={() => (zoom > 1 ? (zoom -= 1) : (zoom /= 2))} on:keydown>
			<Icon name="zoomOut" />
