...
<script lang="ts">
	...
	let contentsWrapper: HTMLElement;
	...
	const getDistanceToEdges = (): Rectangle => {
		const { top, bottom, left, right } = contentsWrapper.getBoundingClientRect();
		return {
	...
	const getY = ({ bottom, top }: Rectangle) => {
		if (top < 0) return -1 * top;
		if (bottom < 0) return bottom;
		return 0;
	};
	const getX = ({ left, right }: Rectangle) => {
	...
	const openPopover = async () => {
		isOpen = true;
		await tick();
		const dist = getDistanceToEdges();
		translateX = getX(dist);
		translateY = getY(dist);
	};

	$: {
		console.log(typeof triggerWidth, triggerWidth);
		console.log(typeof triggerHeight, triggerHeight);
		console.log(typeof popover, popover);
	}
</script>
