...
<section>
	...
	<div class="wrap" on:click={disabled ? null : handleClick} on:keydown>
		{#if split.projectId && Number(split.projectId.toString()) != Number('0x00')}
			<li>
				<p><b>Project ID:</b></p>
				{split.projectId}
			</li>
		...
		{#if split.lockedUntil}
			<li>
				<p><b>Locked:</b></p>
				<p>until {formatDate(Number(split.lockedUntil) * 1000, 'yyyy-MM-dd')}</p>
			</li>
		{/if}
	</div>
</section>
